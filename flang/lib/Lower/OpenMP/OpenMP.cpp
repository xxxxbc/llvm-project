...
/// metadirective variant's generated region as private, mimicking
...
static void markSequentialLoopIVs(
  ...
  for (lower::pft::Evaluation &nested : eval.getNestedEvaluations()) {
    ...
    // such constructs. Nested unresolved metadirectives are not treated
    // specially because privatizing a sibling loop index here is benign.
    ...
    // Skip indices that already have a DSA. This includes predetermined
    // indices (the variant's associated loops and nested loop-construct
    // indices) and indices named in a clause.
...
/// loop is not a canonical DO loop.
...
markMetadirectiveLoopIVs(semantics::SemanticsContext &semaCtx,
  ...
  for (int64_t level = 0; level < affectedDepth; ++level) {
    ...
    // is rejected by the metadirective loop-nest semantic checks
    // (check-omp-variant.cpp). Those checks are a companion patch, so guard
    // here too: bail out for the caller to emit a TODO rather than crash on a
    // non-canonical loop if the checks are absent.
  ...
  // Variants that generate a parallel, task, or (since OpenMP 5.2) teams
  // region privatize the index of every nested sequential loop. The body of
  // `doEval`, the innermost associated loop, contains those loops.
  ...
  if (privatizesNestedSeqLoopIVs) {
    // Indices named in the variant's clauses already have an explicit DSA.
    // Do not mark them here.
